	...
	server.registerTool(
		"read_elements",
		{
			...
			inputSchema: {
				...
				mode: z
					...
					.describe("Read mode: 'summary' for statistics, 'full' for all elements, 'byIds' for specific elements, 'byRange' for index range."),
				elementIds: z.array(z.string()).optional().describe("Element IDs to read (required for 'byIds' mode)"),
				startIndex: z.number().optional().describe("Start index for range read (required for 'byRange' mode)"),
				endIndex: z.number().optional().describe("End index for range read (required for 'byRange' mode)"),
			},
		},
		async ({ path, mode, elementIds, startIndex, endIndex }) => {
			...
			try {
				...
				switch (mode) {
					case "summary": {
						result = { summary: getElementSummary(elements), totalElements: elements.length };
						break;
					}
					case "full": {
					...
					case "byIds": {
						if (!elementIds || elementIds.length === 0) throw new Error("elementIds is required for 'byIds' mode");
						const idSet = new Set(elementIds);
						const filtered = elements.filter((el) => idSet.has(el.id));
						result = { elements: filtered, found: filtered.length, requested: elementIds.length };
						break;
					}
					case "byRange": {
						if (startIndex === undefined || endIndex === undefined) {
							throw new Error("startIndex and endIndex are required for 'byRange' mode");
						}
						const start = Math.max(0, startIndex);
						const end = Math.min(elements.length, endIndex);
						const sliced = elements.slice(start, end);
						result = { elements: sliced, range: { start, end }, totalElements: elements.length };
						break;
					}
				}

				await log("info", "read_elements", { path, mode }, `Read elements (mode: ${mode})`);

				return {
					content: [
						{
							type: "text",
							text: JSON.stringify({ success: true, path: fullPath, ...result } as Record<string, unknown>, null, 2),
						},
					],
				};
			} catch (error) {
