		...
		"append_elements",
		{
			...
			inputSchema: {
				...
				elements: z
					.array(z.any())
					.describe(
						"Array of element objects to add. Design system defaults will be applied automatically.",
					),
			},
		},
		async ({ path, elements }) => {
			...
			try {
				...
				const newElements = await Promise.all(
					rawElements.map((el) => applyDesignSystemDefaults(el)),
				);
				const mergedElements = mergeElements(drawing.elements, newElements, {
					regenerateIds: true,
				});
				...
				await log(
					"info",
					"append_elements",
					{ path, elementCount: newElements.length },
